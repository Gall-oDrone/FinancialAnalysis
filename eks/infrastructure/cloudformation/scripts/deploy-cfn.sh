...
upload_template_to_s3() {
  ...
  aws s3 cp "$template_file" "s3://${bucket_name}/${template_key}" --region "$AWS_REGION"
...
deploy_iam_stack() {
  ...
  if stack_exists "$IAM_STACK_NAME"; then
    aws cloudformation update-stack --stack-name "$IAM_STACK_NAME" --template-url "$template_url" \
      --capabilities CAPABILITY_NAMED_IAM --parameters "ParameterKey=ParentStackName,ParameterValue=$STACK_NAME" \
      --region "$AWS_REGION" 2>/dev/null && wait_for_stack "UPDATE" "$IAM_STACK_NAME" || true
  else
...
deploy_main_stack() {
  ...
  if stack_exists "$STACK_NAME"; then
    aws cloudformation update-stack --stack-name "$STACK_NAME" --template-url "$template_url" \
      ...
      --region "$AWS_REGION" 2>/dev/null && wait_for_stack "UPDATE" "$STACK_NAME" || true
  else
...
deploy_cloudfront_stack() {
  ...
  if stack_exists "$CLOUDFRONT_STACK_NAME"; then
    aws cloudformation update-stack --stack-name "$CLOUDFRONT_STACK_NAME" --template-url "$template_url" \
      ...
      --region "$AWS_REGION" 2>/dev/null && wait_for_stack "UPDATE" "$CLOUDFRONT_STACK_NAME" || true
  else
    aws cloudformation create-stack --stack-name "$CLOUDFRONT_STACK_NAME" --template-url "$template_url" \
      --parameters \
        "ParameterKey=ParentStackName,ParameterValue=$STACK_NAME" \
        "ParameterKey=InstancePublicDnsName,ParameterValue=$instance_dns" \
        "ParameterKey=PriceClass,ParameterValue=$CLOUDFRONT_PRICE_CLASS" \
