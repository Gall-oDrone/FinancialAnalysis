...
$jupyterCmd = @'
cd /app && pip install jupyter -q && jupyter notebook --ip=0.0.0.0 --port=8888 --no-browser --allow-root --notebook-dir=/app --NotebookApp.token='' --NotebookApp.password='' --NotebookApp.allow_origin='*'
'@
...
if (Test-Path $EnvFile) {
    $composeArgs += @("--env-from-file", $EnvFile, "-v", "${EnvFile}:/app/.env:ro")
}

$composeArgs += @("-e", "PGDBHOST=postgres")
